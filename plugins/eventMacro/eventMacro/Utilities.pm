	...
	if ( $kw =~ /^\/(.*?)\/(\w?)$/ ) {
		if ( $text =~ /$1/ || ( $2 eq 'i' && $text =~ /$1/i ) ) {
			if ( !defined $cmpr ) {
				no strict;
				foreach my $idx ( 1 .. $#- ) {
					$eventMacro->set_scalar_var( ".lastMatch$idx", ${$idx} );
				}
				use strict;
			}
			return 1;
...
sub getnpcID {
	...
	foreach my $npc ( @{ $npcsList->getItems() } ) {
		if ( $what == 1 ) {
			return $npc->{binID} if ( $npc->{pos}{x} == $a && $npc->{pos}{y} == $b );
		}
		elsif ( $what == 2 ) {
			if ( $npc->{name} =~ /$a/ || ( $b eq "i" && $npc->{name} =~ /$a/i ) ) {
				push @ids, $npc->{binID};
			}
		}
		else {
			return $npc->{binID} if $npc->{name} eq $a;
		}
	}
...
sub getInventoryIDs {
	...
	foreach my $item ( @{ $char->inventory->getItems } ) {
		if ( lc( $item->name ) eq $find || $item->{nameID} == $find ) {
			push @ids, $item->{binID};
		}
	}
...
sub getInventoryTypeIDs {
	...
	my @ids;
	foreach my $item ( @{ $char->inventory->getItems } ) {
		if ( $item->usable() eq 1 && $find eq "usable" ) {
			push @ids, $item->{binID};
		}
		if ( $item->equippable() eq 1 && $item->{equipped} eq 0 && $find eq "equip" ) {
			push @ids, $item->{binID};
		}
		if ( $item->usable() eq 0 && $item->equippable() eq 0 && $find eq "etc" ) {
			push @ids, $item->{binID};
		}
		if ( $item->{type} eq 6 && $find eq "card" ) {
			push @ids, $item->{binID};
		}
	}
	unless (@ids) { push @ids, -1 }
...
sub getInventoryAmount {
	...
	my $amount = 0;
	foreach my $item ( @{ $char->inventory->getItems } ) {
		if ( lc( $item->name ) eq $arg || $item->{nameID} == $arg ) {
			$amount += $item->{amount};
		}
	}
	return $amount;
...
sub getShopAmount {
	...
	foreach my $articleItem (@::articles) {
		...
		if ( lc( $articleItem->{name} ) eq $arg || $articleItem->{nameID} == $arg ) {
			$amount += $articleItem->{quantity};
		}
	}
	return $amount;
}
...
sub find_variable {
	...
	if ( my $accessed_var = find_accessed_variable($text) ) {
		return (
			{
				display_name => $accessed_var->{display_name},
				type         => $accessed_var->{type},
				real_name    => $accessed_var->{real_name},
				complement   => $accessed_var->{complement}
			}
		);
	}
...
sub find_accessed_variable {
	...
	if ( $text =~ /^\$($valid_var_characters)(\[|\{)(.+)(\]|\})/ ) {
		...
		my $type =
		  ( $open_bracket eq '[' ? 'accessed_array' : 'accessed_hash' );
